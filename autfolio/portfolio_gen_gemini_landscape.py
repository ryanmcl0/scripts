#!/usr/bin/env python3
# Enhanced PDF generator with smart collaging and adaptable page sizes
import os
import re
import sys
import time
    ...
    try:
        if isinstance(image_path_or_buffer, io.BytesIO):
            ...
            with PILImage.open(image_path_or_buffer) as img:
                ...
                image_path_or_buffer.seek(0)  # Reset for later use
        else:
            with PILImage.open(image_path_or_buffer) as img:
                width, height = img.size
...
def create_image_row(image_data_list, max_width, max_height, custom_margin=None):
    ...
    # Calculate the ideal height for images in this row and cap it
...
def process_image_section(image_lines, styles, story, missing_files, optimization_stats, max_content_width, max_content_height):
    """
    ...
    if dji_verticals:
        ...
        for i in range(0, len(dji_verticals), 4):
            ...
            if len(chunk) == 2:
                ...
                row_layout = create_image_row(chunk, max_width, max_content_height, custom_margin=DJI_DIPTYCH_MARGIN)
                if row_layout:
    ...
    if full_width_images:
        ...
        for image in full_width_images:
            section_layouts.append(create_image_row([image], max_width, max_content_height))

    # Priority 3: Panoramics.
    if panoramics:
        print(f"      -> Processing {len(panoramics)} panoramic image(s).")
        for pano in panoramics:
            section_layouts.append(create_image_row([pano], max_width, max_content_height))

    # Priority 4: DJI Landscapes.
    if dji_landscapes:
        print(f"      -> Processing {len(dji_landscapes)} landscape DJI image(s).")
        for dji_land in dji_landscapes:
            section_layouts.append(create_image_row([dji_land], max_width, max_content_height))

    # Finally, process all remaining images from this section.
    if others:
        remaining_layouts = create_layouts_for_remaining_images(others, max_width, max_content_height, optimization_stats)
...
def process_lines_with_collaging(lines, styles, story, missing_files, optimization_stats, max_content_width, max_content_height):
    """
    ...
    while i < total_lines:
        ...
        if is_image_line(line):
            ...
            process_image_section(image_section, styles, story, missing_files, optimization_stats, max_content_width, max_content_height)
            continue
...
class DarkPageTemplate(PageTemplate):
    def __init__(self, id, frames, page_size, document_title="", date="", header_font='Arial', **kwargs):
    ...
    def beforeDrawPage(self, canvas, doc):
        ...
        canvas.drawString(50, page_height - 12, self.date)
        if self.document_title:
            canvas.drawCentredString(page_width / 2, page_height - 12, self.document_title)
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, total_pages):
        ...
        self.drawRightString(page_width - 50, 10, f"{page_num}/{total_pages}")
...
def build_pdf(input_file, output_file, page_size, date="June 2024"):
    ...
    try:
        ...
        process_lines_with_collaging(lines, styles, story, missing_files, optimization_stats, frame_width, frame_height)
        ...
        if total_images > 0:
            ...
            if optimization_stats['optimized_count'] > 0 and total_images > 0:
        ...
        # Create a canvasmaker that passes the page_size to our NumberedCanvas
        canvas_maker = lambda *args, **kwargs: NumberedCanvas(page_size, *args, **kwargs)
...
def main():
    ...
    date = "January - February 2025"  # Change this to your desired date

    print("=== Optimized Markdown to PDF Converter with Smart Collaging ===")
    print(f"Page Size: {page_size_name} ({page_size[0]/mm:.1f}mm x {page_size[1]/mm:.1f}mm)")
    ...
    success, build_time = build_pdf(input_file, output_file, page_size, date)
    end_time = time.time()
    ...
    if success:
        ...
        print("🎨 Images have been intelligently collaged for optimal presentation.")
        if PIL_AVAILABLE:
        ...
        print("   • Maintained image quality and readability")
    else:
